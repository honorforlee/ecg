%% ECG 2 R Peaks Detection / Method of local maxima
clear all; close all;
...
threshold = (max(ecg_norm_2([1:10000]))-mean(ecg_norm_2([1:10000])))/3 + mean(ecg_norm_2);

%% Using findpeaks

[R_peak_findpeaks,R_peak_loc_findpeaks] = findpeaks(ecg_norm_2,'MinPeakHeight',threshold); 
% peak is the amplitude and peak_loc is the location of the R waves

hold on;
plot(R_peak_loc_findpeaks/Fs_2,ecg_norm_2(R_peak_loc_findpeaks),'rv','MarkerFaceColor','r','MarkerSize',8);
plot([0 max(t)], threshold*[1 1],'r--')
legend('ECG signal','R waves','Threshold');

%% Using self-made function

R_peak_loc_self = find(ecg_norm_2 > threshold); % find ecg samples > threshold computed
...
xlim([0 4]);
hold on;
plot(R_peak_loc_self_final/Fs_2,ecg_norm_2(R_peak_loc_self_final),'rv','MarkerFaceColor','r','MarkerSize',8);
plot([0 max(t)], threshold*[1 1],'r--')
legend('ECG signal','R waves','Threshold');
...
for i = 1:length(R_peak_loc_findpeaks)-1
    d(i) = (R_peak_loc_findpeaks(i+1)-R_peak_loc_findpeaks(i))/Fs_2; % Time distance between two consecutive R waves
    d_mean = mean(d(i));
end
...
N1 = 1000000;
F=([0:N1-1]/N1-0.5)*Fs_2;
v_fft = abs(fft(v(1:20*Fs_2),N1)).^2;
