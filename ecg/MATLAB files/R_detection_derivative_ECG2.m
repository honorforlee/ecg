%% ECG 2 R Peaks Detection / Method of the derivative
...
ecg_deriv = diff(ecg_norm_2);

threshold_deriv = (max(ecg_deriv([1:10000]))-mean(ecg_deriv([1:10000])))/2;
threshold = (max(ecg_norm_2([1:10000]))-mean(ecg_norm_2([1:10000])))/2;
...
hold on;
plot(peak_loc_max/Fs_2,ecg_deriv(peak_loc_max),'rv','MarkerFaceColor','r');
plot(peak_loc_min/Fs_2,-ecg_deriv_inv(peak_loc_min),'rv','MarkerFaceColor','r');
...
R_peak_amp = ecg_norm_2(R_peak_loc);
...
hold on;
plot(R_peak_loc/Fs_2,ecg_norm_2(R_peak_loc),'rv','MarkerFaceColor','r');
